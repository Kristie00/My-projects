...
datespine_join as (
    ...
    where c.repository_account is not null
    and c.valid_to_datetime_utc is null
...
github as (
    ...
    from {{ ref('int_githubb') }} g
        inner join {{ ref('int_gsheet') }} c
            on g.organization_name = c.organization
...
final as (
    ...
    from github g
        right join datespine_join d
            on d.date_quarter = extract(datetime from g.first_day_of_period)
            and d.organization_name = g.organization_name

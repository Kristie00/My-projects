...
datespine_join as (
    ...
    from datespine d
        cross join {{ ref('int_gsheet') }} c
    where c.valid_to_datetime_utc is null
    and c.tags_unnested is not null
...
stackoverflow as (
    ...
    select
        ...
        s.organization_name,
        count(distinct s._pk) as post_count,
        ...
        round(sum(case when s.accepted_answer_id is not null then 1 else 0 end) / count(distinct s._pk), 3) as accepted_answer_count,
        sum(case when s.answer_count = 0 or s.answer_count is null then 1 else 0 end) as no_answer_count,
        round(sum(case when s.answer_count = 0 or s.answer_count is null then 1 else 0 end) / count(distinct s._pk), 3) as avg_no_answer_count,
        round(avg(s.score), 3) as score,
        sum(array_length((select array_agg(columns) from
            (
            select * from unnest(split(s.tags_unnested, '|'))
                except distinct
            select * from unnest(split(c.tags_unnested, ', '))
            ) as columns))) as tags_count,
        max(s.last_activity_date) as last_activity_datetime_utc,
        max(s.last_edit_date) as last_edit_datetime_utc

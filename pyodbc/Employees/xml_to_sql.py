...
try:
    ...
    with open('employees.xml', 'r', encoding='utf-8') as fileXML:
        ...
        cols = ['first_name', 'last_name', 'birth_date', 'branch', 'gender', 'salary_by_year', 'position']
        rows = []
        ...
        for node in rootXML:
            ...
            positionTag = node.find('position').text if node is not None else None
            birthTag = node.find('birth_date').text if node is not None else None
            genderTag = node.find('gender').text if node is not None else None
            yearTag = node.find('salary_by_year').text if node is not None else None
        ...
        XMLdf['salary_by_year'] = pandas.to_numeric(XMLdf['salary_by_year'], downcast="integer")
        print(XMLdf)
        print(cols)
        print(rows)
